...
export class UsersController {
  ...
  @Get()
  async getUsers(
    @Query() query: FindUsersQueryModel,
  ): Promise<UserViewModelPage> {
  ...
  async createUser(@Body() body: CreateUserDto): Promise<UserViewModel> {
    ...
    if (!createdUserId)
      throw new HttpException('error', HTTP_Status.BAD_REQUEST_400);
  ...
  async deleteUser(@Param('id') userId) {
    ...
    if (!isDeletedUser)
      throw new HttpException('user not found', HTTP_Status.NOT_FOUND_404);
  }
}
